...
for i in range(1,n_range+1):
    for j in range(t_range+1):
        for k in range(2):
            ...
            os.close(temp)
            s1 = subprocess.check_output("python3 generator.py " + str(i) + " " + str(j) +  " " + sat_unsat + " test.txt > correct.txt", stdin = data, shell = True)
            if k == 0:
                s2 = subprocess.check_output("time python3 " + testfile + " test.txt > my.txt", stdin = data, shell = True)
                ...
                with open('test_output' +'.txt', 'a') as op:
                    op.write('__________________________________________________________________')
                ...
                s2 = subprocess.check_output("time python3 " + testfile + " test.txt > my.txt", stdin = data, shell = True)
                s3 = subprocess.check_output("time python3 " + testfile + " test.txt", stdin = data, shell = True)
                time = s2.decode("utf-8")
                unsat = s3.decode("utf-8")
                with open('test_output' +'.txt', 'a') as op:
                    op.write('__________________________________________________________________')
